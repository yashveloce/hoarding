...
function Media_Type_Master() {
    ...
    const columns = [
        ...
        <button type="button" className="btn btn-warning" data-toggle="tooltip" title="Edit" onClick={() => { onEdit(params.row) }} ><i className="bi bi-pencil-fill"></i></button>

        <button style={{ marginLeft: '20%' }} className="btn btn-danger" data-toggle="tooltip" title="Delete" onClick={() => { onDelete(params.row.id) }}><i className="bi bi-trash-fill"></i></button>
                        ...
                        <div className='card card-primary card-outline' style={{ padding: '20px' }}>
                            <form className="form-group" onSubmit={onModalFormSubmit}>
                                ...
                                <div className="field col-md-4 text-right">
                                        <label>id</label>
                                        <input defaultValue={modelMediaType.id} onChange={onModalInputChange} className="form-control" style={{ marginTop: '10px' }} name="id" type="text" placeholder='Enter media type' required />
                                    </div>
                                    <div className="field col-md-4 text-right">
                                        <label>Media Types</label>
                                        <input defaultValue={modelMediaType.media_type} onChange={onModalInputChange} className="form-control" style={{ marginTop: '10px' }} name="media_type" type="text" placeholder='Enter media type' required />
                                    </div>
                                </div>
                                <div className="field" style={{ width: '100%', textAlign: 'center', marginTop: '20px' }}>
                                    <button className="btn btn-primary" type='submit' style={{ marginLeft: '80px' }}>Save</button>

                                    <button className="btn btn-primary" type='reset' style={{ marginLeft: '70px' }} >Reset</button>
                                </div>
                            </form>
                        </div>
                    </Modal.Body>
                            ...
                            <h3 style={{ width: '100%', textAlign: 'center' }}>MEDIA TYPE MASTER</h3>
                                ...
                                <form className="form-group" onSubmit={onFormSubmit}>
                                    <div className="row">
                                        ...
                                        <div className="field col-md-4 text-right">
                                            <label>Media Types</label>
                                            <input placeholder="enter media types" onChange={onInputChange} className="form-control" style={{ marginTop: '10px' }} name="media_type" type="text"  required />
                                        </div>
                                    </div>
                                    <div className="field" style={{ width: '100%', textAlign: 'center', marginTop: '20px' }}>
                                        <button className="btn btn-primary" type='submit' style={{ marginLeft: '80px' }}>Save</button>
